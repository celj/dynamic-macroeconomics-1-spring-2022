...
\begin{reamrk}
The only condition for choosing the policy instruments is that both the social security and the unemployment insurance systmes be self-financing.
\end{remark}
...
$$
\sum_{j} \sum_{y} \sum_{s} \mu_{j} \lambda_{j}(y, s)\left[C_{j}(y, s)+Y_{j}(y, s)\right]=f(K, N)+(1-\delta) \sum_{j} \sum_{y} \sum_{s} \mu_{j} \lambda_{j}(y, s) Y_{j-1}(y, s)
$$
